...
class LeftOuterJoinAssociationTest < ActiveRecord::TestCase
  fixtures :authors, :essays, :posts, :comments, :categorizations
  ...
  def test_construct_finder_sql_does_not_table_name_collide_on_duplicate_associations
    assert_nothing_raised do
      sql = capture_sql { Person.left_outer_joins(:agents => {:agents => :agents}).left_outer_joins(:agents => {:agents => {:primary_contact => :agents}}).to_a }[1]
  ...
  def test_join_conditions_added_to_join_clause
    ...
    assert_match(/"essays"."writer_type".*?=.*?\?/i, sql)
